"""
NAB — LSTM Anomali Tespiti (DİAGNOSTİK + NAB STANDARDI — v11)
Karami et al. (2025) arXiv:2510.11141

v10 → v11 değişiklikleri:
  1. F1=0 sebep TEŞHİSİ:
     - "no_anom_test"   : Test'te anomali yok (NAB std: F1=1.0 if no FP)
     - "missed_all"     : Test'te anomali var, hiç tahmin yok
     - "all_fp"         : Çok fazla FP, hiç TP yok
     - "partial_miss"   : TP var ama az
     - "ok"             : Normal F1>0

  2. İKİ EVALUATION:
     - Strict F1: standart (anomalisiz → 0)
     - NAB F1  : anomalisiz dosyada hiç FP yoksa → 1.0 (makale standardı)
...
print(f"Eval   : Strict F1 + NAB F1 (anomalisiz=1.0 if no FP)\n")
...
def compute_metrics_diagnostic(y_true, y_pred):
    """
    F1 + TEŞHİS etiketi döndürür.
    
    Diagnostic codes:
      "no_anom_test"  : Test'te anomali yok (test_anom=0)
        - hiç FP yoksa → strict F1=0, NAB F1=1.0 ✓ ideal
        - FP varsa     → strict F1=0, NAB F1=0   ✗ false alarm
      "missed_all"    : Test'te anomali var, hiç tahmin yok (preds.sum()=0)
      "all_fp"        : Çok tahmin var ama TP=0 (model anomaliyi kaçırdı, başka şey buldu)
      "partial_miss"  : TP var, F1 < 0.1 (çok az TP)
      "low_f1"        : 0.1 ≤ F1 < 0.3
      "mid_f1"        : 0.3 ≤ F1 < 0.6
      "high_f1"       : F1 ≥ 0.6
    """
    ...
    # NAB standart: anomalisiz dosyada hiç FP yoksa F1=1.0
    test_has_anom = (y_true.sum() > 0)
    if not test_has_anom:
        if fp == 0:
            nab_f1 = 1.0
            diag = "no_anom_test_clean"
        else:
            nab_f1 = 0.0
            diag = "no_anom_test_FP"
    else:
        nab_f1 = strict_f1
        if y_pred.sum() == 0:
            diag = "missed_all"
...
print(f"Toplam CSV: {len(csv_files)}"); print("="*100)
print(f"{'Dosya':<42} {'Anom T/V/Te':<13} {'F1':>6} {'NAB':>6} {'Pre':>5} {'Rec':>5} {'MAE':>6} {'Teşhis':<20}")
print("-"*100)
results=[]
for path,key in csv_files:
    try:
        ...
        if res is None: 
            print(f"  {key.split('/')[-1][:40]:<42}  ATLANDI")
            continue
        results.append(res)
        anom_str = f"{res['train_anom']}/{res['val_anom']}/{res['test_anom']}"
        fname = res["key"].split("/")[-1][:40]
        print(f"  {fname:<40}  {anom_str:<13} {res['f1']:>6.3f} {res['nab_f1']:>6.3f} "
              f"{res['pre']:>5.3f} {res['rec']:>5.3f} {res['mae']:>6.3f}  {res['diag']:<20}")
...
print(f"\n{'='*100}")
print(f"  F1=0 SEBEP ANALİZİ")
print(f"{'='*100}")
diag_counts = rdf["diag"].value_counts()
diag_descs = {
    "no_anom_test_clean" : "Test'te anomali yok, hiç FP yok (NAB'da F1=1.0)",
    "no_anom_test_FP"    : "Test'te anomali yok ama FP var (yanlış alarm)",
    "missed_all"         : "Test'te anomali var, model hiç tahmin yapmadı (τ çok yüksek)",
    "all_fp"             : "Çok tahmin var ama TP=0 (model yanlış yerleri buldu)",
    "partial_miss"       : "TP var ama çok az (F1 < 0.10)",
    "low_f1"             : "F1: 0.10-0.30",
    "mid_f1"             : "F1: 0.30-0.60",
    "high_f1"            : "F1: 0.60+",
}
...
# ── EVALUATION KARŞILAŞTIRMA ──
print(f"\n{'='*100}")
print(f"  EVALUATION KARŞILAŞTIRMA (Strict vs NAB Standart)")
print(f"{'='*100}")
print(f"  Strict F1 ortalaması   : {rdf['f1'].mean():.4f}  (anomalisiz dosyalar 0 sayılır)")
print(f"  NAB    F1 ortalaması   : {rdf['nab_f1'].mean():.4f}  (anomalisiz + hiç FP yok = 1.0)")
print(f"  Makale F1              : {PAPER_OVERALL['f1']:.4f}")

# Sadece anomalili dosyalarda
anom_only = rdf[rdf["is_anomaly"]]
print(f"\n  Sadece anomalili dosyalar ({len(anom_only)} dosya):")
print(f"    Strict F1 ortalaması : {anom_only['f1'].mean():.4f}")
print(f"    Precision ortalaması : {anom_only['pre'].mean():.4f}")
print(f"    Recall    ortalaması : {anom_only['rec'].mean():.4f}")
print(f"    FPR       ortalaması : {anom_only['fpr'].mean():.4f}  (makale Table II: 0.215)")

# ── KATEGORİ BAZLI ──
print(f"\n{'='*100}")
print(f"  KATEGORİ BAZLI (NAB F1 ile)")
print(f"{'='*100}")
print(f"{'Kategori':<18} {'n':>3} {'Strict F1':>10} {'NAB F1':>8} {'MAE':>7}  {'Mak.F1':>7}")
print("-"*70)
for cat_name in CAT_MAP.values():
    ...
    print(f"  {cat_name:<16} {len(sub):>3} {sub['f1'].mean():>10.4f} "
          f"{sub['nab_f1'].mean():>8.4f} {sub['mae'].mean():>7.4f}  {mf1:>7}")

rdf.to_csv(os.path.join(NAB_ROOT,"lstm_results_v11_diagnostic.csv"),index=False)


# ══════════════════════════════════════════════════════════════════
# GRAFİK: F1=0 TEŞHİS DAĞILIMI
# ══════════════════════════════════════════════════════════════════
fig, axes = plt.subplots(1, 2, figsize=(16, 7))

# Sol: Teşhis kategorileri pie/bar
ax = axes[0]
diag_order = ["no_anom_test_clean", "no_anom_test_FP", "missed_all", 
              "all_fp", "partial_miss", "low_f1", "mid_f1", "high_f1"]
diag_colors = {
    "no_anom_test_clean" : "#4CAF50",  # iyi (yeşil)
    "no_anom_test_FP"    : "#FF9800",  # turuncu
    "missed_all"         : "#F44336",  # kırmızı (sorunlu)
    "all_fp"             : "#E91E63",  
    "partial_miss"       : "#FFC107",  
    "low_f1"             : "#03A9F4",  
    "mid_f1"             : "#2196F3",  
    "high_f1"            : "#0D47A1",  # koyu mavi (iyi)
}
...
ax.set_title("F1=0 Sebep Analizi — Dosya Teşhis Dağılımı\n"
             f"Toplam {total} dosya, τ={TAU} sabit",
             fontsize=12, fontweight="bold")
ax.grid(True, alpha=0.3, axis="x")
ax.invert_yaxis()

# Sağ: Strict vs NAB F1
ax = axes[1]
...
plt.suptitle("LSTM — NAB Diagnostic Analysis (v11)", fontsize=13, fontweight="bold", y=1.00)
plt.tight_layout()
plt.savefig(os.path.join(NAB_ROOT,"viz_diagnostic_v11.png"), dpi=150, bbox_inches="tight")
plt.show()
print(f"\nDiagnostic grafik kaydedildi: viz_diagnostic_v11.png")
